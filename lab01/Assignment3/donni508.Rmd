...
```
```{r 3.1 comment}
# data_PIdiabetes <- read.csv("pima-indians-diabetes.csv")
```
...
- From the glm model, Both Plasma glucose and Age have a significant influence on Diabetes. From the results of this classification, we can see that our misclassification error is about 26.6%. This is not too bad I think  
...
ggplot(data = data_PIdiabetes_r02, aes(x = X8, y = X2, colour = X10)) + 
  ...
  labs(x = "Age (years)", y = "Plasma glucose concentration", colour  = "Predicted Diabetes (0=no or 1=yes)", title = "r = 0.2" )
...
ggplot(data = data_PIdiabetes_r08, aes(x = X8, y = X2, colour = X10)) + 
  ...
  labs(x = "Age (years)", y = "Plasma glucose concentration", colour  = "Predicted Diabetes (0=no or 1=yes)", title = "r = 0.8" )
