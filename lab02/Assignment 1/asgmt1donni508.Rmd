...
ggplot(data = data.frame(pred_test_1se, obs = data1_test[,102]))+
  geom_point(aes(x = 1:nrow(data1_test), y = lambda.1se, color = "prediction"))+
  geom_point(aes(x = 1:nrow(data1_test), y = obs, color = "observation"))+
  labs(x  = "ID of different samples", y = "Fat level")

```

- We can see that the differences between points of observation and prediction value is close. So we consider it a good prediction.
